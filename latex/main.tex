...
\begin{table}[]
	\begin{tabular}{|l|c|c|c|c|c|}
		...
		Fedcoin~\cite{GR16}	& 1 &     2 (1?)   &  1 (?)   &      2     & 1          \\ \hline
			e-krona~\cite{KRONA} 	& 2&     0     &   0      &        -1   &    0       \\ \hline
		...
		Chaum \etal ~\cite{CGM21}		&1(?)  &   2     &    2     &     2      &    0     \\ \hline
...
\end{table}
 ...
 \paragraph{P $\rightarrow$ D:} Privacy enthusiasts oppose indirect collection and monetization of their personal information
   ...
   \begin{itemize}
 	...
 	\item \textbf{0:} D can collect exclusive payments' data. %define data
...
\end{itemize}
 ...
 \subsection*{Notes}
 \subsubsection{Evaluation Framework Explanations}
\paragraph{Cash:} L can track transactions to a certain extent (a transaction can be observed at the time it happens, banknotes can be tracked using their serial numbers). L does not have direct means to collect and learn the transaction information (amount, payer payee). This also means that D does not have access to exclusive data and this results in more parity between data holders. 
 
\paragraph{Payment network:} L can recover nearly all transactions as all transactions are recorded. The payment network collects exclusive data, which is in line with the current data collection schemes.

\paragraph{FedCoin:} It is aimed to be a substitute for cash. Cash conversions are anonymous but Fed knows the public address of a Fedcoin recipient. As the recipient's address is not hidden,  L can recover partial information. Fed can apply a threshold for the amount transacted in order to meet AML requirements.

\paragraph{e-krona:} E-krona is similar to the payment networks. As for the conflict between the data holders, there will be less parity than the status quo, since e-krona operates on a permissioned blockchain.

\paragraph{Bitcoin:} Users remain anonymous as long as they are not associated with the address they use. Pseudonimity enables L to recover data partially. As bitcoin blockchain is public, data holders cannot gather exclusive data and as a result, there is more parity among data holders. It is also less costly to provide required transaction information to L.

\paragraph{Zcash:} As the Zcash transactions are confidential, L cannot recover the complete set of payments to and from Alice. 
%(D to L explanation!!)

\paragraph{PRCash:} The transactions are anonymous below a certain threshold. L can recover the transactions over this threshold. The cryptographic techniques used make it more expensive for D to maintain the database and provide data to L.
...
% \paragraph{Choi \etal~\cite{CHLRS21}:} Similar to PRcash
 ...
 %\paragraph{Chaum \etal~\cite{CGM21}:} a token-based (ex: banknotes), software-only CBDC without DLT. The token-based CBDC proposed here also allows the preservation of a key feature of physical cash: transaction privacy. provides , quantum-resistant transaction privacy  and AML parties are not required to reveal their identities at any time during the transaction; both can remain anonymous. However, the payee must be able to verify the token’s authenticity. The CBDC proposed here is “software-only”, merely a smartphone app that does not  require any additional hardware from users In our proposed architecture, all consumer and merchant interactions are with commercial banks. However, money creation and the database are provided exclusively by  the central bank. The commercial banks authenticate the customers when they withdraw  CBDC and the merchants/payees when they receive CBDC, but when spending CBDC, the  customers/payers only need to authorize their transactions and do not need to identify themselves A key objective of our architecture is to ensure that central banks do not have to interact directly with customers or retain any information about them, merely maintaining a  list of spent coins. The authentication is delegated to commercial banks. Finally, the total system cost is low. In the proposed scheme, central banks do not learn the identities of consumers or merchants or the total transaction amounts. . Commercial banks continue to provide crucial customer and merchant authentication and, in particular, remain the guardians of KYC information. Commercial banks observe when merchants receive funds and can limit the amount of CBDC per transaction that an individual merchant may receive, come transparency  for AML
...
% ``With CAD-coin, the central bank again acts as the gateway to conversion from central bank money to CAD-coin, but privacy at conversion is not required.  The privacy requirements with CAD-coin instead relate to the needs of the non-central bank participants.  In CAD-coin, banks are still identified by a public address, but in order to complete payments requests generated from the real system, which are in terms of a bank’s legal name, a bank must know the complete list mapping of bank names to public addresses in the distributed ledger. Therefore, the only way to ensure privacy of the complete transaction record of a bank is to limit each individual bank’s access to the distributed ledger itself and to either (i) not involve all parties in transaction validation or (ii) find a way of validating the trades without seeing them, a so called “zero-knowledge validation”.~\cite{GR16}
 ...
 \section{Discussion}
 \begin{itemize}
 	\item Are  P $\rightarrow$ L and  L $\rightarrow$ P mutually exclusive?
 	\item Do we need -1, -2? Measure against status quo.
 	\item D $\rightarrow$ D: exchange vs other data holders
 	\item If central banks hold all the data, law enforcement can get the data directly without going to commercial banks.
 	\item For D$\rightarrow$ D and D$\rightarrow$ L: should we divide the column as central bank and commercial banks?
 \end{itemize}
